...
\cmd{TextWidth}  & 15.5 & Largeur du bloc de texte (\cmd{textwidth}). \\
\addlinespace

\cmd{TextHeight} & 24.5 & Hauteur du bloc de texte (\cmd{textheight}). \\
\addlinespace

\cmd{ColSep}     & 0.05 & Séparation entre colonnes (\cmd{colsep}). \\
\addlinespace
...
\cmd{Offset}     & 0.0 & Décalage horizontal : active \cmd{offset}, fixe
\cmd{hoffset} à la valeur, remet \cmd{voffset} à 0. \cmd{off} désactive. \\
\addlinespace

\cmd{Hoffset}    & 1.5 \textbf{(!)} & Décalage horizontal seul (\cmd{offset} +
\cmd{hoffset}). Le défaut hérité est \cmd{vignwidth\_default} (1.5),
probablement un copier-coller — préciser toujours une valeur. \\
\addlinespace

\cmd{Voffset}    & 1.5 \textbf{(!)} & Décalage vertical seul (\cmd{offset} +
\cmd{voffset}). Même remarque que \cmd{Hoffset}. \\
\end{longtable}
