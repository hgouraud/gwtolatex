...
let test_tree_width tree =
  let rec loop first w0 tree =
    match tree with
    | [] -> true
    | row :: tree -> let w = row_width row in
        if first then loop false w tree
        else if w <> w0 then false
        else loop false w0 tree
  in loop true 0 tree
...
let print_tree tree =
  if not (test_tree_width tree) then (
    Printf.eprintf "Unbalanced tree \n"; exit 1);
